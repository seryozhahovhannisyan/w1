...
<link rel="stylesheet" href="<%=request.getContextPath()%>/css/wallet/wallet_mobile/wallet_send_money_transfer_popup_mobile.css">
...
function transfer(){
    $(".make_transfer").click(function (){
        ...
        params.data = data;
        WALLET.request_transaction_make_payment(params, request_money_make_payment_callback, error_handler);
    });
}
...
//                        '<p  draggable="true" ondragover="allowDrop(event)" ondrop="drop(event)" >' + file.name + '</p>');
                }
...
function select_blur() {
    $(".request_transaction_trans").click(function () {
        ...
        if (($(".popup_ammount_input").val()).trim() != "") {
            ...
            data.currencyType = $("#currencyType option:selected").val();
            params.data = data;
            WALLET.send_money_check_tax(params, request_money_check_tax_callback, wallet_error_handler);
        }
        else{
...
function show_request_money_popup(id, name_surname,user_photo_path) {
    pre_make_payment_view(id, name_surname,user_photo_path);

}
function show_request_money_popup_transfer() {
    var params = {};
    var data = {};
    params.async = true;
    data.userId = $(".popup_sm_header").data("user_id");
    ...
    data.currencyType = $("#currencyType option:selected").val();
    params.data = data;
    var info = $(".popup_info_div").text();
    if ($(".popup_ammount_input").val() != "" && $(".popup_info_div").text()!="") {
        WALLET.request_transaction_preview(params, request_money_preview_callback, error_handler);
    }
    else {
        return false;
    }
}
function logic_show_request_money_popup_previous( ){
    ...
    for (var i = 0; i < params.array_ids.length; i++) {
        if (params.array_ids[i] == params.id && params.array_ids.length >= 2) {
            ...
            if (params.array_ids[i-2] == undefined && previews_id != undefined) {
                show_request_money_popup_previous(previews_id, previews_name_surname, previews_user_photo_path);
...
function logic_show_request_money_popup_next(){
    disable_previews_scroll = false;
    for (var i = params.array_ids.length; i >= 0; i--) {
        if (params.array_ids[i] == params.id && params.array_ids.length >= 2) {
            ...
            if (params.array_ids[i+2] == undefined && next_id != undefined) {
                show_request_money_popup_next(next_id, next_name_surname, next_user_photo_path);
...
function show_request_money_popup_previous(id, name_surname, user_photo_path) {
    close_send_money_popup();
    pre_make_payment_view(id, name_surname, user_photo_path);
}
function show_request_money_popup_next(id, name_surname, user_photo_path) {
    close_send_money_popup();
    pre_make_payment_view(id, name_surname, user_photo_path);
}
...
function scrolling_popup() {
    ...
    $(".container-for-scroll-sliding").scroll(function(event) {
        ...
        if ((scrollleft > need_margin_left + 40) && disable_previews_scroll !== true){
            params.id = logic_show_request_money_popup_previous();
            setTimeout(function() {
                $container.scrollLeft(scrollLeft + need_margin_left);
            }, 2500);
        }
        if ((scrollleft < need_margin_left - 40) && disable_next_scroll !== true){
            params.id = logic_show_request_money_popup_next();
            setTimeout(function() {
                $container.scrollLeft(scrollLeft + need_margin_left);
            }, 2500);
        }
                    ...
                    $.each(search_users, function (i, item) {
                        ...
                        if (search_user != null) {
                            ...
                            $('.div'+ id + ' img').click(function (){
                                show_request_money_popup(id, name_surname, user_photo_path );
...
function pre_make_payment_view(id, name_surname,user_photo_path) {
    ...
    params.data = data;
    params.async = true;
    WALLET.load_available_currencies(params, load_available_currencies_callback, error_handler);
}
...
//CALLBACKS
var error_handler = function (handle) {
...
var request_money_check_tax_callback = function(data){
    loader_hide();
    request_money_preview_callback(data);
};
var request_money_preview_callback = function(data){
...
var request_money_make_payment_callback = function(data){
    loader_hide();
    $(".popup_cont_transfer").hide();
    $(".popup_cont").hide();
};
            ...
            <div class="popup_main">
                <div class="popup_sm_header_parent">
                    <div class="popup_sm_header">
                        ...
                        <div>
                           <span  class="request_transaction_trans"   >
                                <s:text name="wallet.login.request.transaction">Request transaction</s:text>
                            </span>
                            <span class="request_transaction_trans"  >
                                <img src="<%=request.getContextPath()%><%=request.getContextPath()%>/img/wallet/icon/requst_transaction.png"
                                     alt="envelop icon"/>
                            </span>
                        </div>
                    </div>
